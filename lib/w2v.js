...
async function buildVectors(corpusFolder) {
  log.logInfo("Building Phrases", corpusFolder);
  await word2phrase(`${corpusFolder}/${corpus}`, `${corpusFolder}/${phrases}`, phraseParam);

  log.logInfo("Building vectors", corpusFolder);
  // console.log(`${corpusFolder}/${phrases}`, `${corpusFolder}/${vectors}`);
  const r = await word2vec(`${corpusFolder}/${corpus}`, `${corpusFolder}/${vectors}`, vectorParam);
  console.log("word2vec", r);
}
...
async function loadCorpusModel(corpusFolder) {
  log.logInfo("Loading model", corpusFolder);
