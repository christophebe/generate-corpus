...
async function generateCorpus(options) {
  ...
  logInfo(`Found ${infoUrls.length} urls`, options);
...
async function retrieveContent(options, urlInfo) {
  logInfo(`try to retrieve Content : ${urlInfo.url}`, options);
  let proxy = null;
  ...
  try {
    logInfo(`request : ${urlInfo.url}`, searchOptions);
    ...
    logError(`Error during the http request for ${urlInfo.url}`, options, error);
    return { title: urlInfo.title, text: "", url: urlInfo.url };
  }
}
